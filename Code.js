...
const SCHEMA = {
  ...
  /* ===== ใหม่ ===== */
  USERS:           ['Username','PassHash','Salt','FullName','Role','Ward','LineUserId','Active','LastLogin'],
  SESSIONS:        ['Token','Username','Role','FullName','Ward','IssuedAt','ExpireAt'],
  LINE_TARGETS:    ['TargetId','TargetType','Ward','DisplayName','Active','Note'],
  INR_RESULTS:     ['ResultId','AN','TestDate','INR','PT_sec','WarfarinDose','TargetMin','TargetMax','Note','RecordBy']
};
/* ================= AUTH & ROLE ================= */
const SESSION_HOURS = 8;

const PERMISSIONS = {
  admin:        ['ward','scan','summary','report','log','inr','inr_edit','user_admin','line_admin'],
  nutritionist: ['ward','scan','summary','report','inr'],            // นักโภชนาการ
  nurse:        ['ward','summary','report','inr','inr_edit'],        // พยาบาล
  pharmacist:   ['ward','summary','report','inr','inr_edit','log']   // เภสัชกร
};
const ROLE_TH = { admin:'ผู้ดูแลระบบ', nutritionist:'นักโภชนาการ',
                  nurse:'พยาบาล', pharmacist:'เภสัชกร' };
...
function login_(username, password){
  const u = readAll_('USERS').filter(x=>
        String(x.Username).toLowerCase() === String(username||'').toLowerCase())[0];
  if (!u)  throw new Error('ไม่พบชื่อผู้ใช้นี้');
  if (String(u.Active).toUpperCase() !== 'YES') throw new Error('บัญชีถูกระงับการใช้งาน');
  if (hash_(password, u.Salt) !== u.PassHash)   throw new Error('รหัสผ่านไม่ถูกต้อง');

  const token = Utilities.getUuid().replace(/-/g,'');
  const exp = new Date(Date.now() + SESSION_HOURS*3600*1000);
  append_('SESSIONS', {
    Token: token, Username: u.Username, Role: u.Role, FullName: u.FullName, Ward: u.Ward,
    IssuedAt: now_(), ExpireAt: Utilities.formatDate(exp, TZ, 'yyyy-MM-dd HH:mm:ss')
  });
  // อัปเดต LastLogin
  const sh = sheet_('USERS'), vals = sh.getDataRange().getValues();
  for (let i=1;i<vals.length;i++) if (String(vals[i][0])===String(u.Username)) sh.getRange(i+1,9).setValue(now_());

  log_(u.Username,'LOGIN', u.Role, 'เข้าสู่ระบบสำเร็จ', 'SUCCESS');
  return { token: token, username: u.Username, fullName: u.FullName, role: u.Role,
           roleTH: ROLE_TH[u.Role]||u.Role, ward: u.Ward, perms: PERMISSIONS[u.Role]||[],
           expireAt: Utilities.formatDate(exp, TZ, 'yyyy-MM-dd HH:mm') };
}

function auth_(token){
  if (!token) throw new Error('AUTH_REQUIRED');
  const s = readAll_('SESSIONS').filter(x=> x.Token === token)[0];
  if (!s) throw new Error('AUTH_REQUIRED');
  if (new Date(s.ExpireAt) < new Date()) throw new Error('SESSION_EXPIRED');
  return s;
}
function need_(sess, perm){
  const p = PERMISSIONS[sess.Role] || [];
  if (p.indexOf(perm) < 0) throw new Error('สิทธิ์ไม่เพียงพอ ('+(ROLE_TH[sess.Role]||sess.Role)+' ไม่สามารถใช้เมนูนี้ได้)');
}
function logout_(token){
  const sh = sheet_('SESSIONS'), v = sh.getDataRange().getValues();
  for (let i=v.length-1;i>=1;i--) if (String(v[i][0])===String(token)) sh.deleteRow(i+1);
  return { ok:true };
}
function createUser_(o){
  const salt = Utilities.getUuid().slice(0,12);
  append_('USERS', { Username:o.username, PassHash:hash_(o.password,salt), Salt:salt,
    FullName:o.fullName, Role:o.role, Ward:o.ward||'', LineUserId:o.lineUserId||'',
    Active:'YES', LastLogin:'' });
  return { ok:true };
}
/* ================= LINE MESSAGING API ================= */
...
/** ดึงปลายทาง: กลุ่ม/ผู้ใช้ ที่ Active และตรงหอผู้ป่วย (Ward='*' = ทุกหอ) */
function lineTargets_(ward){
  return readAll_('LINE_TARGETS')
    .filter(t=> String(t.Active).toUpperCase()==='YES' &&
               (String(t.Ward)==='*' || String(t.Ward)===String(ward)))
    .map(t=> t.TargetId);
}

function pushLine_(toIds, messages){
  const tk = lineToken_();
  if (!tk) return { sent:0, skip:'NO_TOKEN' };
  if (!toIds.length) return { sent:0, skip:'NO_TARGET' };
  let sent = 0, errs = [];
  toIds.forEach(to=>{
    try{
      ...
      else errs.push(to+':'+res.getResponseCode()+' '+res.getContentText().slice(0,120));
    }catch(e){ errs.push(to+':'+e); }
  });
  return { sent: sent, errors: errs };
}

/** Flex Message การ์ดแจ้งเตือนวาร์ฟาริน */
function warfarinFlex_(a){
  const colors = { HIGH:'#D92C2C', MODERATE:'#E08600', LOW:'#2E9E4F', NONE:'#5B6875' };
  const c = colors[a.RiskLevel] || '#D92C2C';
  const row = (l,v)=> ({ type:'box', layout:'baseline', spacing:'sm', contents:[
      { type:'text', text:l, color:'#8C96A0', size:'sm', flex:3 },
      { type:'text', text:String(v||'-'), wrap:true, color:'#1C2430', size:'sm', flex:7, weight:'bold' }]});
  return [{
    type:'flex',
    altText:'🚨 แจ้งเตือน: คนไข้รายนี้ได้รับยากลุ่มวาร์ฟาริน — AN '+a.AN,
    contents:{ type:'bubble', size:'mega',
      header:{ type:'box', layout:'vertical', backgroundColor:c, paddingAll:'14px', contents:[
        { type:'text', text:'🚨 WARFARIN FOOD ALERT', color:'#FFFFFF', weight:'bold', size:'sm' },
        { type:'text', text:'คนไข้รายนี้ได้รับยากลุ่มวาร์ฟาริน', color:'#FFFFFF', weight:'bold', size:'lg', wrap:true }]},
      body:{ type:'box', layout:'vertical', spacing:'md', contents:[
        row('ผู้ป่วย', a.PatientName),
        row('AN', a.AN),
        row('หอ/เตียง', a.Ward + ' / เตียง ' + a.Bed),
        row('มื้ออาหาร', a.MealType),
        { type:'separator', margin:'md' },
        { type:'text', text:'เมนูที่สั่ง', size:'xs', color:'#8C96A0', margin:'md' },
        { type:'text', text:String(a.MenuText||'-'), size:'sm', wrap:true },
        { type:'box', layout:'vertical', margin:'md', backgroundColor:'#FFF0F0',
          cornerRadius:'8px', paddingAll:'10px', contents:[
          { type:'text', text:'⚠️ อาหารที่มีปฏิกิริยา ('+a.RiskLevel+')', size:'xs', color:c, weight:'bold' },
          { type:'text', text:String(a.MatchedFoods||'ไม่พบ'), size:'sm', wrap:true, margin:'sm' }]},
        { type:'text', text:'ผู้ตรวจสอบ: '+(a.ActionBy||'-')+'  •  '+a.Timestamp,
          size:'xxs', color:'#8C96A0', margin:'md' }]},
      footer:{ type:'box', layout:'vertical', contents:[
        { type:'button', style:'primary', color:c, height:'sm',
          action:{ type:'uri', label:'เปิดข้อมูลผู้ป่วย',
                   uri:(PropertiesService.getScriptProperties().getProperty('WEBAPP_URL')||'https://example.com')+'?an='+a.AN }}]}
    }
  }];
}

function notifyWarfarin_(alertRow){
  const r = pushLine_(lineTargets_(alertRow.Ward), warfarinFlex_(alertRow));
  log_(alertRow.ActionBy||'system','LINE_PUSH', alertRow.AN,
       'ส่ง '+r.sent+' ปลายทาง '+(r.errors&&r.errors.length? '| ERR: '+r.errors.join(' / '):''),
       r.sent? 'SUCCESS':'ERROR');
  return r;
}

/** แจ้งเตือนค่า INR ผิดช่วงเป้าหมาย */
function notifyINR_(pt, inr, min, max, by){
  const abn = inr > max ? 'สูงกว่าเป้าหมาย (เสี่ยงเลือดออก)' : 'ต่ำกว่าเป้าหมาย (เสี่ยงลิ่มเลือด)';
  const c   = inr > max ? '#D92C2C' : '#E08600';
  const msg = [{ type:'flex', altText:'⚡ INR ผิดปกติ AN '+pt.AN,
    contents:{ type:'bubble',
      header:{ type:'box', layout:'vertical', backgroundColor:c, paddingAll:'12px', contents:[
        { type:'text', text:'⚡ INR ALERT', color:'#fff', weight:'bold' }]},
      body:{ type:'box', layout:'vertical', spacing:'sm', contents:[
        { type:'text', text:pt.FullName+' (AN '+pt.AN+')', weight:'bold', wrap:true },
        { type:'text', text:pt.Ward+' เตียง '+pt.Bed, size:'sm', color:'#8C96A0' },
        { type:'text', text:'INR = '+inr, size:'xxl', weight:'bold', color:c, margin:'md' },
        { type:'text', text:'เป้าหมาย '+min+'–'+max+'  →  '+abn, size:'sm', wrap:true },
        { type:'text', text:'บันทึกโดย '+by, size:'xxs', color:'#8C96A0', margin:'md' }]}}}];
  const r = pushLine_(lineTargets_(pt.Ward), msg);
  log_(by,'LINE_INR_ALERT', pt.AN, 'INR='+inr+' ('+abn+') ส่ง '+r.sent, r.sent?'SUCCESS':'ERROR');
  return r;
}
/* ================= INR ================= */
function saveINR_(o, by){
  const id = uid_('INR');
  append_('INR_RESULTS', { ResultId:id, AN:o.AN, TestDate:o.TestDate, INR:Number(o.INR),
    PT_sec:o.PT_sec||'', WarfarinDose:o.WarfarinDose||'', TargetMin:Number(o.TargetMin||2.0),
    TargetMax:Number(o.TargetMax||3.0), Note:o.Note||'', RecordBy:by });
  log_(by,'SAVE_INR', o.AN, 'INR='+o.INR+' @'+o.TestDate, 'SUCCESS');

  const min = Number(o.TargetMin||2.0), max = Number(o.TargetMax||3.0), v = Number(o.INR);
  let pushed = { sent:0 };
  if (v < min || v > max){
    const pt = readAll_('PATIENTS').filter(x=> String(x.AN)===String(o.AN))[0];
    if (pt) pushed = notifyINR_(pt, v, min, max, by);
  }
  return { resultId:id, notified:pushed.sent };
}

function inrSeries_(an){
  const rows = readAll_('INR_RESULTS').filter(x=> String(x.AN)===String(an))
    .sort((a,b)=> new Date(a.TestDate) - new Date(b.TestDate));
  const vals = rows.map(r=> Number(r.INR)).filter(n=> !isNaN(n));
  const inRange = rows.filter(r=> Number(r.INR) >= Number(r.TargetMin) &&
                                  Number(r.INR) <= Number(r.TargetMax)).length;
  return {
    rows: rows,
    stat: {
      n: rows.length,
      last: rows.length ? Number(rows[rows.length-1].INR) : null,
      ...
      ttr: rows.length ? Math.round(inRange/rows.length*100) : 0   // Time in Therapeutic Range (%)
    }
  };
}
function handle_(p){
  const cb = p.callback;
  const OPEN = ['login'];                       // action ที่ไม่ต้อง auth
  let sess = { Username:'anonymous', Role:'guest', FullName:'-', Ward:'' };
  try {
    if (OPEN.indexOf(p.action) < 0) sess = auth_(p.token);
    const user = sess.Username;
    let data;

    switch (p.action) {
      /* ---- AUTH ---- */
      case 'login':   data = login_(p.username, p.password); break;
      case 'logout':  data = logout_(p.token); log_(user,'LOGOUT','','ออกจากระบบ','SUCCESS'); break;
      case 'me':      data = { username:sess.Username, fullName:sess.FullName, role:sess.Role,
                               roleTH:ROLE_TH[sess.Role], ward:sess.Ward,
                               perms:PERMISSIONS[sess.Role]||[] }; break;
      case 'addUser': need_(sess,'user_admin'); data = createUser_(p.payload);
                      log_(user,'ADD_USER', p.payload.username, p.payload.role,'SUCCESS'); break;

      /* ---- MASTER ---- */
      case 'bootstrap':
        need_(sess,'ward');
        data = { patients:readAll_('PATIENTS'), foodMap:readAll_('FOOD_MAP'),
                 drugPair:readAll_('DRUG_PAIR_MAP'),
                 me:{ username:sess.Username, fullName:sess.FullName, role:sess.Role,
                      roleTH:ROLE_TH[sess.Role], ward:sess.Ward, perms:PERMISSIONS[sess.Role]||[] } };
        log_(user,'BOOTSTRAP','','load master data','SUCCESS'); break;
      ...
      case 'patient': {
        ...
        data = { patient:pt,
          ...
          alerts:readAll_('ALERT_LOG').filter(x=>String(x.AN)===an),
          inr:inrSeries_(an) };
        log_(user,'VIEW_PATIENT', an,'open patient summary','SUCCESS'); break;
      }
      ...
      case 'saveMeal':
        need_(sess,'scan');
        p.payload.OrderId = p.payload.OrderId || uid_('MO');
        p.payload.OrderBy = sess.Username;
        append_('MEAL_ORDERS', p.payload);
        log_(user,'SAVE_MEAL', p.payload.AN, p.payload.MealType+' | '+p.payload.MenuText,'SUCCESS');
        data = { orderId: p.payload.OrderId }; break;

      case 'saveAlert': {
        need_(sess,'scan');
        const a = p.payload;
        a.AlertId = a.AlertId || uid_('AL');
        a.Timestamp = now_();
        a.ActionBy = sess.FullName + ' ('+ (ROLE_TH[sess.Role]||sess.Role) +')';
        a.Result = 'NOTIFIED';
        append_('ALERT_LOG', a);
        const push = notifyWarfarin_(a);            // 🔔 ยิง LINE อัตโนมัติ
        data = { alertId:a.AlertId, lineSent:push.sent, lineSkip:push.skip||'' }; break;
      }

      /* ---- INR ---- */
      case 'saveINR':  need_(sess,'inr_edit'); data = saveINR_(p.payload, sess.FullName); break;
      case 'inr':      need_(sess,'inr');      data = inrSeries_(p.an); break;

      /* ---- REPORT / LOG ---- */
      case 'report': need_(sess,'report'); data = buildWarfarinReport_();
                     log_(user,'REPORT','warfarin_ward','generate report','SUCCESS'); break;
      case 'logs':   need_(sess,'log');    data = readAll_('SYSTEM_LOG').slice(-300).reverse(); break;
      case 'testLine':
        need_(sess,'line_admin');
        data = pushLine_(lineTargets_(p.ward||'*'),
                 [{type:'text', text:'✅ ทดสอบการเชื่อมต่อ Warfarin Alert System — '+now_()}]);
        break;

      default: return out_({ ok:false, error:'unknown action: '+p.action }, cb);
    }
    return out_({ ok:true, data:data }, cb);
  } catch (err){
    const m = String(err.message || err);
    log_(sess.Username||'anonymous', p.action||'UNKNOWN','', m, 'ERROR');
    return out_({ ok:false, error:m, auth: /AUTH_REQUIRED|SESSION_EXPIRED/.test(m) }, cb);
  }
}
function SETUP_PART2(){
  ['USERS','SESSIONS','LINE_TARGETS','INR_RESULTS'].forEach(k=> sheet_(k));

  [['admin','1234','ผู้ดูแลระบบ IT','admin','*'],
   ['nutri01','1234','น.ส.ปิยะดา โภชนากร','nutritionist','*'],
   ['nurse01','1234','พว.สุนีย์ ห่วงใย','nurse','อายุรกรรมหญิง 1'],
   ['nurse02','1234','พว.อรทัย ดูแลดี','nurse','CVT Ward'],
   ['pharm01','1234','ภญ.กษิญา บัวจันทร์','pharmacist','*']
  ].forEach(u=> createUser_({username:u[0],password:u[1],fullName:u[2],role:u[3],ward:u[4]}));

  [['Cxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx','group','*','กลุ่ม LINE พยาบาลทุกหอ','YES','แทนที่ด้วย groupId จริง'],
   ['Uxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx','user','อายุรกรรมหญิง 1','พว.สุนีย์ ห่วงใย','YES','']
  ].forEach(t=> append_('LINE_TARGETS',{TargetId:t[0],TargetType:t[1],Ward:t[2],
      DisplayName:t[3],Active:t[4],Note:t[5]}));

  [['AN6900125','2026-08-25',1.4,3.0],['AN6900125','2026-08-27',1.9,3.0],
   ['AN6900125','2026-08-29',2.4,3.0],['AN6900125','2026-08-31',3.6,3.0],
   ['AN6900128','2026-08-21',1.2,3.5],['AN6900128','2026-08-24',2.1,3.5],
   ['AN6900128','2026-08-27',2.8,3.5],['AN6900128','2026-08-30',3.1,3.5]
  ].forEach(r=> append_('INR_RESULTS',{ResultId:uid_('INR'),AN:r[0],TestDate:r[1],INR:r[2],
      PT_sec:'',WarfarinDose:'',TargetMin:r[3]===3.5?2.5:2.0,TargetMax:r[3],
      Note:'seed',RecordBy:'system'}));

  log_('system','SETUP2','ALL','users + line targets + INR seed','SUCCESS');
}
